...
- `netlify/functions/telegram.js` - код бота для Netlify.
...
`TELEGRAM_WEBHOOK_SECRET` защищает webhook: Telegram будет отправлять этот секрет в специальном заголовке, а функция будет отклонять чужие POST-запросы.
...
```text
https://api.telegram.org/botВАШ_ТОКЕН/setWebhook?url=https://ваш-сайт.netlify.app/.netlify/functions/telegram&secret_token=ВАШ_WEBHOOK_SECRET
```
