...
const FAQ = [
  ...
  [
    ...
    "Да. Все тексты лежат в netlify/functions/telegram.mjs и легко редактируются.",
  ],
];
...
export default async (request) => {
  if (request.method !== "POST") {
    return new Response("Telegram bot webhook is ready.", { status: 200 });
  }
...
async function handleUpdate(update, token) {
  if (update.message) {
    ...
    await handleMessage(token, chatId, userId, text, username);
    return;
  }
...
async function handleMessage(token, chatId, userId, text, username) {
